...
import { availableLendersTableHead, availableLenders } from '../../../data';
...
const Lend = ({handleDropDownTabClick}: {handleDropDownTabClick: (tab: string) => void}) => {
  ...
  const currentAvailableLenders = availableLenders.slice(indexOfFirstProduct, indexOfLastProduct);
        ...
        <div className="w-full overflow-x-scroll custom-scrollbar">
          <table className="min-w-full custom-scrollbar">
            ...
            <tbody className="divide-y divide-neutral-200">
              {currentAvailableLenders.map(item => (
                <tr
                  key={item.id}
